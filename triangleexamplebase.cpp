...
{
	VkResult resXXX = swapChain.queuePresent(queue, currentBuffer, semaphores.renderComplete);
  

  if (!((resXXX == VK_SUCCESS) || (resXXX == VK_SUBOPTIMAL_KHR))) {
		if (resXXX == VK_ERROR_OUT_OF_DATE_KHR) {
			// Swap chain is no longer compatible with the surface and needs to be recreated
			windowResize();
			return;
		} else {
			VK_CHECK_RESULT(resXXX);
		}
	}
	VK_CHECK_RESULT(vkQueueWaitIdle(queue));
}
